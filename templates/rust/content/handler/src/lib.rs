...
use wasmcp_wasi::http::{Request, Response};
use wasmcp_wasi::keyvalue::Store;
...
impl AsyncToolHandler for WeatherTool {
    ...
    async fn execute_async(args: serde_json::Value) -> Result<String, String> {
        ...
        let geocoding_response = wasmcp_wasi::http::send(geocoding_request).await
        ...
        let weather_response = wasmcp_wasi::http::send(weather_request).await
